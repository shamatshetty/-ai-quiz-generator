...
export default function LeaderboardTab({
  ...
  return (
    <div className="space-y-6 animate-tab-enter text-white">
        ...
        {currentUserRank && (
          <div className="flex items-center gap-3 p-3 rounded-2xl bg-purple-950/50 border border-purple-500/30 shrink-0">
            <div className="w-10 h-10 rounded-xl bg-purple-600 text-white flex items-center justify-center font-heading font-bold text-sm shadow-md shadow-purple-500/30">
              {currentUserRank.rank ? `#${currentUserRank.rank}` : '—'}
            </div>
            <div>
              ...
              <div className="text-sm font-heading font-bold text-white">
                {currentUserRank.quizzesPlayed > 0
      ...
      {/* 2. Podium Display for Top Learners */}
      {top3.length > 0 ? (
        ...
        <div className="dashboard-card p-8 text-center bg-slate-900/50 border border-slate-800">
          <p className="text-slate-300 font-heading font-bold text-base">No quiz records yet.</p>
          <p className="text-xs text-slate-400 mt-1">
            Registered students who complete quizzes will appear here on the podium!
          </p>
        </div>
      )}
              ...
              {filteredList.length > 0 ? (
                ...
                <tr>
                  <td colSpan="6" className="py-8 text-center text-slate-400">
                    <p className="font-semibold text-sm">No registered student quiz attempts found.</p>
                    <p className="text-xs text-slate-500 mt-1">Take a quiz to appear on the leaderboard!</p>
                  </td>
                </tr>
              )}
            </tbody>
